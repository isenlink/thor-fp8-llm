...
| 10240 页（20 GB） | 33.5 GB | ❌ 加载中途进程静默消失（日志 24 行截断、无错误行 = 被 SIGKILL） |
...
⚠️ 另注：本机的池值可能有多处持久化配置（`/etc/sysctl.conf` 与 `/etc/sysctl.d/*.conf` 里各写了一份），
重启后以数值**最大**的那份生效——排查"重启后池变了"时先 `grep -r nr_hugepages /etc/`。
