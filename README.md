...
| 9-15 Qwen3.6-35B-A3B（Q4_K_M MoE，分支） | pp512 **739.98 t/s** / tg128 **40.24 t/s** —— 达到 4B 稠密模型的速度但容量大 9 倍；GQA 极不对称（2 个 KV 头）⇒ 256K f16 KV 仅 **20 GB**；风冷峰值 71°C → [详见](docs/06-benchmarks/thor03-qwen36-35b-a3b-moe-2026-09-15.md) |
| 9-23 Occamy-1.0 200K KV（Q4_K_M MoE，分支） | **`-c 200000`（`n_ctx_slot=200192`）实测可用**：35,651 token 的 prompt 完整接收（32768 基线装不下）；prefill 694–705 t/s；短上下文 decode **50.4 t/s 与 32K 基线 50.11 持平**（容量扩 6.1×、速度不掉），34K 上下文 decode **43.9 t/s（−12.5%）**；⚠️ 负面结果：把 27B 的 DFlash2 草稿挂上来会在投机初始化 `GGML_ASSERT(ggml_can_repeat)` 崩溃，需 `qwen35moe` 兼容草稿 → [详见](docs/06-benchmarks/thor04-occamy-moe-200k-context-2026-09-23.md) |
...
docs/
  ...
  06-benchmarks/          各阶段基准数据与复现命令（含 200K 基准台账）、**基准方法论（三个口径陷阱 + 配对设计）**、**投机解码全矩阵**、**多 slot 并发 + KV/前缀复用实测**、社区同款板实测汇总
