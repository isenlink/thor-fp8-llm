...
- 社区配方逐一证伪：GGML_MMVQ_MAX=2（对 NVFP4 无效）、DFlash2（量化目标模型上 acceptance 崩到 12-32%）⚠️**此条已于 2026-09-16 撤回**、K12（旧栈 200K 反转劣于 K7）——教训：**配方迁移必须核对"模型+栈"双重前提**
  > ⚠️ **修订（2026-09-16）**：DFlash2 那条**判错了**——当时同时改了两个变量（目标模型被重编码损坏 + 换草稿），
  > "acceptance 崩"的真因是目标模型数值损坏，不是 DFlash2。配对重测：DFlash2 全面快于内置 MTP（全类中位 +8.1%，代码类 +28.3%）。
  > 详见 `dflash2-revalidation-2026-09-16.md`。
...
| ~~DFlash2 块扩散起草~~ ⚠️**本行已于 2026-09-16 撤回** | ~~两个目标模型全慢，acc 崩~~ → 真因是**目标模型数值损坏**（A2 重编码），非 DFlash2 本身 | 06-实验6/7 → 见 `dflash2-revalidation-2026-09-16.md` |
