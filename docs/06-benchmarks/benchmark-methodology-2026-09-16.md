...
| 提示词集 | **多类内容 × 多条不同题**（我们最终用 6 类 × 5 题 = 25 题），避免只用 1-2 条 |
...
| 预算 | 25 题 × 3 配置 + 分批重启 ≈ 40-60 分钟（含装载） |
...
3. **长跑分批重启**：运行期存在"每新提示词 ~626 MB"的常驻内存增长（见
   `05-system-tuning/runtime-memory-growth-2026-09-16.md`），
   25 条不同提示词连续跑会 OOM ⇒ **每 10 条不同提示词重启一次 server**（重复同一提示词不受影响）。
