// c58_v9f_host_prod.cpp — c58_v9f_host 的 **H-mem 判别臂副本**（板端 aarch64；Driver API + dlopen）
//   mode=stream  : 与归档宿主逐行同（线性 gB，chunks 段相同 stage）⇒ 用来复现 gran1 的读数（本臂锚点）
//   mode=prodlay : gB 改按**生产寻址**排布（32 行 × row_stride、每 64-K 记录 36 B=[4B sf][32B nib]），
//                  payload 4 B 单元按 canonical 落位放好 ⇒ 内核侧只换取数寻址、smem/数值不变。
//                  记录里的 4 B sf 填 0：本臂**不发行 sf 流**（生产腿的 sf 由 warp0 另发 128 请求/chunk，
//                  占 11% 请求且命中同一 cache line ⇒ 已在卡片 §7 记录为已知简化，判据阈 ±170/220 远大于其影响）。
// 用法: c58_v9f_host_prod {stream|prodlay|stream1|stream1b} <cubin> <chunks/block> <grid> <timeout_ms> [flags] [bw 1] [breps]
...
#include "t4_prodaddr_layout.h"   // CPR / ROW_STRIDE / b_smem_off（与生产头编译期对齐）
...
int main(int argc, char** argv) {
  ...
  const bool prodlay = (strcmp(mode, "prodlay") == 0);
  ...
  if (!prodlay && strcmp(mode, "stream") != 0 && order_mode == 0) { printf("{\"tag\":\"bad_mode\",\"mode\":\"%s\"}\n", mode); return 1; }
  bind();
  ...
  // 口径分母恒为 chunks×grid×4096（与 gran0/gran1 完全同口径）；prodlay 只是**多占**记录里的 4 B sf
  const size_t total_bytes = (size_t)chunks * (size_t)grid * B_CHUNK;
  const size_t tiles = (size_t)((chunks + t4prod::CPR - 1) / t4prod::CPR);
  const size_t alloc_bytes = prodlay ? (size_t)grid * tiles * (size_t)t4prod::TILE_BYTES : total_bytes;
  CUdeviceptr d_gB = 0, d_D = 0, d_diag = 0, d_ts = 0;
  if (p_cuMemAlloc(&d_gB, alloc_bytes) != CUDA_SUCCESS) { printf("{\"tag\":\"stream\",\"ok\":false,\"stage\":\"alloc_gB\",\"bytes\":%zu}\n", alloc_bytes); return 2; }
  ...
  std::vector<uint8_t> host_buf(alloc_bytes);
  if (prodlay) {
    // 一个 n-tile（32 行 × row_stride）的镜像：records[cc][r] 的 payload 4 B 单元 ← canonical stage
    std::vector<uint8_t> tile((size_t)t4prod::TILE_BYTES, 0);
    for (int row = 0; row < 32; ++row)
      for (int u = 0; u < 8; ++u) {
        const uint8_t* s0 = stage.data() + t4prod::b_smem_off(row, u);
        for (int r = 0; r < 4; ++r)
          for (int cc = 0; cc < t4prod::CPR; ++cc) {
            uint8_t* d = tile.data() + (size_t)row * t4prod::ROW_STRIDE + (size_t)cc * t4prod::CHUNK_ROWS +
                         (size_t)r * t4prod::REC + t4prod::src_off_in_rec(u);
            memcpy(d, s0 + r * B_SUB, 4);
          }
      }
    for (size_t b = 0; b < (size_t)grid; ++b)
      for (size_t t = 0; t < tiles; ++t)
        memcpy(host_buf.data() + (b * tiles + t) * (size_t)t4prod::TILE_BYTES, tile.data(), (size_t)t4prod::TILE_BYTES);
    printf("{\"tag\":\"prodlay\",\"cpr\":%d,\"row_stride\":%d,\"tile_bytes\":%d,\"tiles_per_block\":%zu,"
           "\"alloc_bytes\":%zu,\"metric_bytes\":%zu}\n",
           t4prod::CPR, t4prod::ROW_STRIDE, t4prod::TILE_BYTES, tiles, alloc_bytes, total_bytes);
    fflush(stdout);
  ...
  p_cuMemcpyHtoD(d_gB, host_buf.data(), alloc_bytes);
  p_cuCtxSynchronize();
  ...
  if (argc > 7 && atoi(argv[7]) == 1) {
    ...
    if (p_cuModuleGetFunction(&fnbw, mod, "k_b9f_readbw") == CUDA_SUCCESS) {
      unsigned long long n16 = (unsigned long long)(alloc_bytes / 16);
      ...
      {   // 批量计时（单发只有 1 ms 分辨率，不可用）
        ...
        const double by = (double)alloc_bytes * (double)breps;
